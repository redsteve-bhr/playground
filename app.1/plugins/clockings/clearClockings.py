...
class _TransactionsDialog(itg.Dialog):
    ...
    def __populateMenu(self, menu):
        ...
        if self.__totalUnsent > 0:        
            menu.appendRow(_('Clear transactions'), cb=self.__onClearClockings, hasSubItems=True)
        else:
            menu.appendRow(_('No unsent transactions'))
    
    def __onDetails(self, pos, row):
        itg.msgbox(itg.MB_OK, '%s: %s' % (row['label'], row['value']))
        
    def __onClearClockings(self, pos, row):
        dlg = _ClearClockingsDialog()
        resID = dlg.run()
...
class _ClearClockingsDialog(itg.WizardDialog):
...
class _WelcomeDialog(itg.Dialog):
    ...
    def onCreate(self):
        ...
        view = itg.MsgBoxView()
        view.setText(_('Please enter date and time up to and including which all unsent transactions will be marked as sent.'))
...
class _ConfirmDialog(itg.PseudoDialog):
    ...
    def run(self):
        ...
        (_resID, count) = itg.waitbox(_('Collecting data...'), trans.getNumberOfUnsentTransactions, (sqlTime,))
        self.sharedData['count'] = count
        self.sharedData['sqlTime'] = sqlTime
        if (count == 0):
            resID = itg.msgbox(itg.MB_OK_CANCEL, _('No unsent transactions up to and including %s.') % localTime)
            if (resID == itg.ID_OK):
                return itg.ID_BACK
            return resID
            
        resID = itg.msgbox(itg.MB_YES_NO_CANCEL, _('Mark transactions up to and including %(date)s (%(numTrans)s transactions) as sent?') % 
...
class _MarkAsSentDialog(itg.PseudoDialog):
    ...
    def run(self):
        ...
        itg.waitbox(_('Marking %d transactions as sent...') % count, trans.markAsSentToDate, (sqlTime,))
        return itg.ID_NEXT
...
class _FinishDialog(itg.Dialog):
    ...
    def onCreate(self):
        ...
        count = self.sharedData['count']
        view = itg.MsgBoxView()
        view.setText(_('%s transactions have been marked as sent.') % count)
        view.setButton(0, _('OK'), itg.ID_NEXT, self.quit)
        self.addView(view)


class ClearClockingsDialog(itg.PseudoDialog):
...
class ClearClockingsAction(dynButtons.Action):
    
    def getName(self):
        return 'clear.clockings'
    
    def getButtonText(self, actionParam, employee, languages):
        return _('Clear Clockings')
    
    def getDialog(self, actionParam, employee, languages):
        return ClearClockingsDialog()

    def getHelp(self):
        return """
        Clear unsent transactions (i.e. mark them as sent).

        This action brings up a dialog which lets the user clear unsent
        transactions.
